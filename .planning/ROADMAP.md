...
3. Add pure project, room, relation, evidence, and snapshot identity derivation from canonical structural inputs; detailed projection emits one room per full workspace package root up to a documented 64-room structural cap.
...
4. Fixture assertions prove a root package or complete per-package Node/npm workspace produces sorted, disjoint, exhaustive rooms and exact aggregated directed cross-room declarations with no same-room self-loop; the twelve-workspace Vibery-shaped fixture produces twelve rooms and exactly seven expected relation directions without product-specific labels or an LLM.
